...
def _select_new_servers(servers: Dict[str, dict]) -> Dict[str, dict]:
    ...
    with _core._lock:
        ...
        for srv_name, srv_cfg in servers.items():
            supports_parallel = _parse_boolish(
            ...
            if current_scope is None:
                ...
                _core._parallel_safe_servers_scoped[(current_scope, key)] = supports_parallel
...
def is_mcp_tool_parallel_safe(tool_name: str) -> bool:
    ...
    with _core._lock:
        ...
        if scope is not None:
            provenance = _core._mcp_tool_server_scoped.get((scope, tool_name))
            if provenance is None:
                return False
            _server_name, key = provenance
            return bool(_core._parallel_safe_servers_scoped.get((scope, key), False))
...
def has_registered_mcp_tools() -> bool:
    ...
    with _core._lock:
        scope = _core._mcp_registry_scope()
        if scope is not None:
            return any(entry_scope == scope for entry_scope, _ in _core._mcp_tool_server_scoped)
...
def get_registered_mcp_server_names() -> set:
    ...
    with _core._lock:
        ...
        if scope is not None:
            return {
                server_name
                for (entry_scope, _tool_name), (server_name, _key) in _core._mcp_tool_server_scoped.items()
...
def registered_mcp_server_matches_config(name: str, expected: dict) -> bool:
    """Return whether a live or lazy server owns exactly ``expected`` config.

    ACP uses this postcondition for its reserved session-bound server so an
    earlier process-global connection cannot be mistaken for the trusted
    launcher.
    """
    with _core._lock:
        ...
        elif key in _core._lazy_server_configs:
            actual = dict(_core._lazy_server_configs[key])
        else:
            return False
