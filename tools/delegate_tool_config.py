...
def _required_timeout(key: str, env_var: str, default: float) -> float:
    """Positive required-supervision timeout from config, env, or default."""
    value = _cfg().get(key)
    if value is not None:
        try:
            ...
            logger.warning(
                f"delegation.{key}=%r is not a valid number; ignoring it",
                value,
            )
...
def _get_required_no_progress_timeout_seconds() -> float:
    """Idle required-child no-progress deadline."""
    return _required_timeout(
        "required_no_progress_timeout_seconds",
        "DELEGATION_REQUIRED_NO_PROGRESS_TIMEOUT_SECONDS",
        _DEFAULT_REQUIRED_NO_PROGRESS_TIMEOUT_SECONDS,
    )

def _get_required_start_timeout_seconds() -> float:
    """Required-child queued-before-start deadline."""
    return _required_timeout(
        "required_start_timeout_seconds",
        "DELEGATION_REQUIRED_START_TIMEOUT_SECONDS",
        _DEFAULT_REQUIRED_START_TIMEOUT_SECONDS,
    )

def _get_required_in_flight_timeout_seconds() -> float:
    """Wider deadline while a required child is inside a tool/provider call."""
    return _required_timeout(
        "required_in_flight_timeout_seconds",
        "DELEGATION_REQUIRED_IN_FLIGHT_TIMEOUT_SECONDS",
        _DEFAULT_REQUIRED_IN_FLIGHT_TIMEOUT_SECONDS,
    )
