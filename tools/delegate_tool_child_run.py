...
def _close_child_once(child: Any) -> bool:
    ...
    if not hasattr(cleanup_lock, "__enter__"):
...
class _Heartbeat:
    ...
    def __init__(self, child: Any, parent_agent: Any, task_index: int):
        ...
        self.last_seen = {"iter": 0, "tool": None, "ts": None, "generation": 0, "stale": 0}
        self.handle = None
        self.stop_event = threading.Event()
        child._delegation_heartbeat_stop = self.stop_event
    ...
    def tick(self):
        ...
        try:
            ...
            child_meaningful_generation = int(child_summary.get("meaningful_progress_generation") or 0)
            ...
            meaningful = bool(
                child_iter > last_seen["iter"]
                or child_tool != last_seen["tool"]
                or child_meaningful_generation > last_seen["generation"]
            )
            if meaningful or activity_advanced:
                last_seen.update(
                    iter=child_iter, tool=child_tool,
                    generation=child_meaningful_generation, stale=0,
                )
        ...
        if required_id:
            with _quiet("Required heartbeat update failed", exc_info=True):
                ...
                progress_generation = int(progress.get("progress_generation") or 0)
                supervision_status = str(progress.get("status") or "running")
