...
def _required_supervision(batch: _Batch) -> Dict[str, Any]:
    """Current required-controller snapshot, best-effort."""
    if not batch.controller_deleg_id:
        return {}
    try:
        from tools.async_delegation import refresh_required_supervision
        return refresh_required_supervision(batch.controller_deleg_id) or {}
    except Exception:
        logger.debug("Required batch supervision refresh failed", exc_info=True)
        return {}

def _run_required_children(
    batch: _Batch, results: list, *, honor_parent_interrupt: bool
) -> None:
    """Run required children without letting an uninterruptible worker retain the outer async slot."""
    supervision = _required_supervision(batch)
    if (
        getattr(batch.parent_agent, "_interrupt_requested", False) is True
        or batch.stop_event.is_set()
        or supervision.get("terminal")
    ):
        for i, _task, child in batch.children:
            results.append(_fabricated_entry(
                i, "interrupted", "Parent agent stopped before required child execution", child,
            ))
        return
    ...
    child_by_index = {i: child for i, _task, child in batch.children}
    try:
        for i, task, child in batch.children:
            try:
                futures[executor.submit(contextvars.copy_context().run, batch.run_child, i, task, child)] = i
            except Exception:
                # The controller worker catches the raised scheduling failure;
                # its interrupt callback owns disposing every child that never started.
                raise
        ...
        while pending:
            ...
            for future in list(pending):
                ...
                if child_id not in cancelled_ids:
                    continue
                pending.remove(future)
                future.cancel()
                entry = _fabricated_entry(
                    idx, "cancelled", "Required delegation child was cancelled by the parent", child,
                    round(time.monotonic() - batch.overall_start, 2),
                )
            ...
            parent_stopped = (
                ...
                and getattr(
                    batch.parent_agent, "_interrupt_requested", False
                )
                is True
            )
            if (
                ...
                for future in pending:
                    ...
                    if future.done():
                        try:
                            ...
                            results.append(_fabricated_entry(idx, "error", str(exc), child_by_index.get(idx)))
                    else:
                        results.append(_fabricated_entry(
                            idx, "interrupted", "Required delegation supervisor stopped the child before it finished",
                            child_by_index.get(idx),
                        ))
                break
            ...
            for future in done:
                ...
                try:
                    ...
                    entry = _fabricated_entry(idx, "error", str(exc), child_by_index.get(idx))
                results.append(entry)
                _report_child_done(
                    batch.parent_agent,
                    getattr(batch.parent_agent, "_delegate_spinner", None),
                    entry,
                    format_batch_tag(batch.live_deleg_id),
                    [task["goal"][:40] for task in batch.task_list],
                    len(batch.task_list),
                    len(batch.task_list) - len(results),
                )
    finally:
        try:
...
_BACKGROUND_NOTES = {
    "one": (
        "Background dispatch is detached: the subagent result normally re-enters in a later turn. "
        "On the ACP surface, if you end your turn, it will be awaited and consolidated before the reply finalizes. "
...
def _dispatch_background(batch: _Batch) -> str:
    ...
    if batch.required:
        for child in child_agents:
            ...
            child._required_delegation_ancestor_binding = (
                str(batch.controller_deleg_id or ""),
                str(getattr(child, "_subagent_id", "") or ""),
            )
            child._delegation_worker_started_event = threading.Event()
    ...
    if batch.required:
        ...
        return tool_error(
            "Required delegation could not be started under supervision: " + rejection,
            delegationAttempt={
                "id": batch.controller_deleg_id,
                "state": "failed_execution",
                "requestedCount": len(goals),
                "spawnedCount": 0,
