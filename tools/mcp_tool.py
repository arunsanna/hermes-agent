...
# Shared connections can be adopted by profiles with different trust policies; keep the
# call-time trust tier alongside the profile-scoped tool overlay.
_server_trust_levels_scoped: Dict[tuple[Optional[str], Any], str] = {}
# Session-bound Switchboard MCP provenance. These sets are populated from the
# trusted parent config and raw server names at registration time; call-time
# policy must never infer trust from the lossy generated registry name.
...
# The legacy sets above remain the unscoped compatibility surface. Under a
# profile multiplexer, the same connection can be adopted by multiple
# profiles, each with its own parent allowlist, so those decisions are kept
# per (connection, registry scope) as well.
_switchboard_parent_scoped_read_only_tools: Dict[tuple[Any, Optional[str]], set[str]] = {}
_switchboard_parent_scoped_raw_tools: Dict[tuple[Any, Optional[str]], set[str]] = {}
_switchboard_parent_scoped_full_servers: set[tuple[Any, Optional[str]]] = set()
_switchboard_parent_scoped_full_tools: Dict[tuple[Any, Optional[str]], set[str]] = {}
...
# Scoped provenance for multiplexed registry overlays. The legacy map above is
# retained for unscoped callers and external plugins.
_mcp_tool_server_scoped: Dict[tuple[Optional[str], str], tuple[str, Any]] = {}
# Profile-aware parallel-call policy; the legacy set remains the unscoped view.
_parallel_safe_servers_scoped: Dict[tuple[Optional[str], Any], bool] = {}
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'registered_mcp_server_matches_config': ('tools.mcp_tool_discovery', 'registered_mcp_server_matches_config'),
    'get_switchboard_parent_read_only_tool_names': ('tools.mcp_tool_registration', 'get_switchboard_parent_read_only_tool_names'),
    'get_switchboard_parent_tool_names': ('tools.mcp_tool_registration', 'get_switchboard_parent_tool_names'),
