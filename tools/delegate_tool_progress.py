...
class _ChildProgressRelay:
    ...
    def __init__(
        ...
        subagent_id, parent_id, depth, model, toolsets, session_ref, parent_agent=None,
    ) -> None:
    ...
    def _note_required_event(self, event_type, tool_name, preview, kwargs):
        """Update the ACP required controller before display relaying.

        Returns the controller-authoritative preview/kwargs so a timeout or
        cancellation that wins a race cannot be overwritten by a late normal
        child completion.
        """
        if not self.required_tracking:
            return preview, kwargs
        relayed_id = str(kwargs.get("subagent_id") or "")
        direct = not relayed_id or relayed_id == str(self.subagent_id or "")
        descendant = bool(relayed_id) and not direct
        if event_type == "subagent.complete" and direct:
            terminal_snapshot = None
            with _quiet("Required child terminal update failed: %s"):
                ...
                terminal_snapshot = note_required_child_terminal(
                    ...
                    activity=str(preview or kwargs.get("summary") or "child completed"),
                    result={
                        "task_index": self.task_index, "child_id": self.subagent_id,
                        "status": str(kwargs.get("status") or "completed"),
            ...
            if isinstance(terminal_snapshot, dict) and terminal_snapshot.get("terminal"):
                winning_status = str(terminal_snapshot.get("status") or "failed")
                requested_status = str(kwargs.get("status") or "completed").strip().lower()
        ...
        meaningful = (
            (
                event_type in {
                    "subagent.text", "subagent_progress", "tool.started", "tool.completed",
                    DelegateEvent.TASK_PROGRESS, DelegateEvent.TASK_TOOL_STARTED, DelegateEvent.TASK_TOOL_COMPLETED,
                }
                and bool(preview or tool_name or kwargs.get("args"))
            )
            or (descendant and event_type == "subagent.heartbeat" and bool(kwargs.get("meaningful")))
            or (descendant and event_type == "subagent.complete")
        )
        with _quiet("Required child activity update failed: %s"):
            from tools.async_delegation import note_required_child_activity
            note_required_child_activity(
                self.subagent_id,
                current_tool=tool_name or kwargs.get("current_tool"),
                activity=str(preview or tool_name or event_type),
                meaningful=meaningful,
                state="running" if event_type == "subagent.start" else None,
            )
        return preview, kwargs
    ...
    def __call__(self, event_type, tool_name: str = None, preview: str = None, args=None, **kwargs):
        preview, kwargs = self._note_required_event(event_type, tool_name, preview, kwargs)
...
def _build_child_progress_callback(
    ...
    required_tracking = bool(
        subagent_id is not None
        and str(getattr(parent_agent, "platform", "") or "").lower() == "acp"
        and getattr(parent_agent, "_delegate_depth", 0) == 0
    )
    if not spinner and not parent_cb and not required_tracking:
        return None
