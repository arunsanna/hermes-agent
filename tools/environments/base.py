...
def get_activity_callback() -> Callable[..., None] | None:
...
def _fire_activity_callback(label: str, *, meaningful: bool) -> None:
    """Report liveness/progress while remaining compatible with old callbacks."""
...
def touch_activity_if_due(state: dict, label: str, *, meaningful: bool = False) -> None:
    ...
    *state* holds ``last_touch``/``start`` monotonic timestamps. Swallows all exceptions."""
    ...
    try:
        _fire_activity_callback(f"{label} ({int(now - state['start'])}s elapsed)", meaningful=meaningful)
    except Exception:
        pass
...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(
        ...
        # The drain helper lives in ``base_output`` after the decomposition.
        # Wrap its collector at this seam so streamed output still counts as
        # meaningful activity without duplicating the drain implementation.
        raw_append = output.append
        output_activity_now = time.monotonic()
        output_activity_state = {
            "last_touch": output_activity_now - 1.0,
            "start": output_activity_now,
            "interval": 1.0,
        }
        ...
        def append_output(text: str) -> None:
            if not text:
                return
            raw_append(text)
            touch_activity_if_due(output_activity_state, "terminal command produced output", meaningful=True)

        output.append = append_output
