...
def active_task_count() -> int:
    """Number of live delegated child units (a batch of N contributes N; a batch with
    no goal list counts 1), including queued required work — unlike slots."""
    with _records_lock:
        return sum(
...
def has_live_for_session(session_key: str = "", origin_ui_session_id: str = "", parent_session_id: str = "") -> bool:
    """Whether a session still owns any ordinary or required live delegation."""
    return bool(_session_records(_LIVE_DELEGATION_STATES, session_key, origin_ui_session_id, parent_session_id))
...
def dispatch_async_delegation_batch(
    *, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    """Dispatch a supervised required batch or an ordinary background unit.

    Required ACP calls remain one owned unit with their same-turn supervision
    contract. Ordinary calls may represent a whole batch or one completion
    group; units from one call share ``slot_key`` and each produces one
    completion event.
    """
    if required:
...
def _sweep_stale_locked(now: float):
    ...
    for record in _records.values():
        if record.get("required"):
            # Required controllers have their own per-child watchdog and
            # terminalization rail; the legacy stale monitor must never call
            # _finalize() on them or publish a legacy completion event.
            continue
...
def interrupt_all(reason: str = "shutdown") -> int:
    """Signal every live ordinary or required delegation to stop (``/stop``, shutdown)."""
    with _records_lock:
        targets = [r for r in _records.values() if r.get("status") in _LIVE_DELEGATION_STATES]
...
def interrupt_for_session(
    ...
    """Signal live ordinary or required delegations owned by ONE ending session to stop."""
    targets = _session_records(_LIVE_DELEGATION_STATES, session_key, origin_ui_session_id, parent_session_id)
