...
def _repair_object_shape(node):
    ...
    if not isinstance(node, dict):
        return node
    # ``properties`` and ``patternProperties`` map arbitrary user property
    # names to schemas; recurse into values without treating those names as
    # schema keywords (e.g. a property literally named ``type``).
    repaired = {}
    for key, value in node.items():
        if key in ("properties", "patternProperties") and isinstance(value, dict):
            repaired[key] = {prop_name: _repair_object_shape(prop_schema)
