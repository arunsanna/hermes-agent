...
# The turn-boundary image stubber removes native pixels before the next request,
# so this cap is for the active vision turn rather than the whole session. Keep
# enough bytes and pixels for readable screenshots while retaining hard provider
# safety limits below.
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    for attempt in range(5):
        if attempt > 0:
            # Fit the long edge exactly on the first pass when the dimension
            # cap is the reason for resizing. Later passes halve as needed for
            # the byte budget; preserve aspect ratio and a 64px floor.
            if (attempt == 1 and max_dimension is not None
...
def _build_native_vision_tool_result(
    ...
    if scale_note:
        text_part += f"\n\nNote: {scale_note}"
    # Preserve a reloadable local source after history unwraps the multimodal
    # envelope; provider URLs and data URLs are not stable local reload paths.
    if isinstance(image_url, str) and not image_url.startswith(("data:", "http://", "https://")):
