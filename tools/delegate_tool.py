...
from tools.delegate_tool_child_run import (  # noqa: F401
    _ChildRun, _attach_child, _build_result_entry, _close_child_once, _dump_subagent_timeout_diagnostic, _fabricated_entry,
...
from tools.delegate_tool_config import (  # noqa: F401
    ...
    _get_max_spawn_depth, _get_orchestrator_enabled, _get_required_in_flight_timeout_seconds,
    _get_required_no_progress_timeout_seconds, _get_required_start_timeout_seconds,
...
def _open_child_session_db(parent_agent) -> Any:
    ...
    with _quiet("subagent: failed to open dedicated SessionDB; child persistence disabled", exc_info=True):
        ...
        if getattr(parent_session_db, "_shared_registry_owned", False):
            from hermes_state_registry import acquire

            return acquire(_parent_db_path) if _parent_db_path is not None else acquire()
        from hermes_state import SessionDB

        return SessionDB(db_path=_parent_db_path) if _parent_db_path is not None else SessionDB()
    return None
...
def _build_child_agent(
    ...
    if isinstance(parent_required_binding, tuple) and len(parent_required_binding) == 2:
...
def _run_single_child(
    ...
    * ``"completed"``       — normal finish. See #97655.
    """
    worker_started_event = getattr(child, "_delegation_worker_started_event", None)
    if hasattr(worker_started_event, "set"):
        worker_started_event.set()

    required_id = getattr(child, "_required_delegation_id", None)
    if required_id:
        with _quiet("Required child pre-start supervision check failed", exc_info=True):
            from tools.async_delegation import refresh_required_supervision
            supervision = refresh_required_supervision(required_id) or {}
            child_id = str(getattr(child, "_subagent_id", "") or "")
            child_snapshot = next(
                (
                    item for item in (supervision.get("children") or [])
                    if str(item.get("child_id") or "") == child_id
                ),
                {},
            )
            if supervision.get("terminal") or child_snapshot.get("terminal"):
                terminal_status = str(
                    child_snapshot.get("status") or supervision.get("status") or "interrupted"
                )
                ...
                return {
                    ...
                    "summary": None,
                    "error": "Required delegation supervisor terminalized the child before provider execution",
                    "api_calls": 0,
                    "duration_seconds": 0,
...
def delegate_task(
    ...
    output_schema: Optional[Dict[str, Any]] = None, required: bool = False,
    ...
    required = bool(
        required
        and str(getattr(parent_agent, "platform", "") or "").strip().lower() == "acp"
    ...
    schema_preflight_rejected = False
    if not err:
        task_schemas, err = _coerce_task_schemas(task_list, output_schema)
        schema_preflight_rejected = err is not None
    if err:
        if schema_preflight_rejected:
    ...
    children, err = _build_children(
        ...
        live_deleg_id=controller_deleg_id if required else live_deleg_id, live_writers=live_writers,
    )
    if err:
        return tool_error(err)
    batch = _Batch(
        ...
        required=required, controller_deleg_id=controller_deleg_id,
    )
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": _p(
                "boolean",
                "DEPRECATED / IGNORED. Top-level single and batch delegations run in the background "
                "automatically; execution is detached. On ACP, work started during the turn is awaited and "
                "consolidated before the reply finalizes; on other surfaces, the result re-enters the "
                "conversation in a later turn. Setting this has no effect and remains only for backward "
                "compatibility.",
            ),
            "action": _p(
                "string",
...
def _model_required_value(parent_agent=None) -> bool:
    """ACP roots require same-turn supervised delegation completion."""
    return (
...
def _required_control(handler_name: str, args: dict, parent_agent=None) -> str:
    """Ownership-scoped controller operations for ACP root turns."""
    from tools import async_delegation as required_controller

    if parent_agent is None:
        return json.dumps({
            "status": "unavailable", "code": "control_context_missing",
            "error": "Required delegation controls are available only to the top-level ACP parent turn.",
        })
    if str(getattr(parent_agent, "platform", "") or "").lower() != "acp":
        return json.dumps({
            "status": "unavailable", "code": "control_not_acp_root",
            "error": "Required delegation controls are available only to the top-level ACP parent turn.",
        })
    if getattr(parent_agent, "_delegate_depth", 0) != 0:
        return json.dumps({
            "status": "unavailable", "code": "control_forbidden_child",
            "error": "Required delegation controls are available only to the top-level ACP parent turn.",
        })

    delegation_id = str(args.get("delegation_id") or "")
    if handler_name == "status":
        payload = required_controller.required_status(parent_agent, delegation_id)
    elif handler_name == "wait":
        payload = required_controller.wait_required(
            parent_agent, delegation_id, timeout_seconds=args.get("timeout_seconds", 0),
        )
    else:
        payload = required_controller.cancel_required(
            parent_agent, delegation_id, child_id=args.get("child_id"),
        )
    if payload.get("terminal"):
        # The durable barrier inserts the canonical result exactly once.
        payload = dict(payload)
        payload.pop("result", None)
        payload["observation_pending"] = not bool(payload.get("consumed"))
...
for _name, _description, _properties in (
    ...
    (
        "delegation_wait",
        "Wait up to 30 seconds for same-turn ACP delegation progress. A terminal result is inserted automatically "
        "through the durable required-delegation barrier.",
        {
            "delegation_id": _REQUIRED_ID_PROPERTY,
    ...
    (
        ...
        {"delegation_id": _REQUIRED_ID_PROPERTY, "child_id": {"type": "string"}},
    ),
):
    registry.register(
        name=_name,
        ...
        schema={
            ...
            "parameters": {
                "type": "object", "properties": _properties, "required": ["delegation_id"],
            },
        },
