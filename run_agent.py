...
def _run_required_interrupt_off_thread(interrupt_fn, agent) -> None:
    """Cancel ACP required delegations without blocking the interrupt caller."""
    try:
...
class AIAgent(
    ...
    def _persist_required_observation_pair(
        ...
        """Atomically persist a required delegation call/result observation."""
        from contextlib import nullcontext
        from agent.context_compressor import _DB_PERSISTED_MARKER

        persist_lock = getattr(self, "_session_persist_lock", None)
        with (persist_lock if persist_lock is not None else nullcontext()):
            if getattr(self, "_persist_disabled", False):
                raise RuntimeError("required delegation observation persistence is disabled")
            session_db = getattr(self, "_session_db", None)
            if session_db is None:
                raise RuntimeError("required delegation observation has no session database")
            if not getattr(self, "_session_db_created", False):
                self._ensure_db_session()
            self._flush_messages_to_session_db_unlocked(
                messages[:observation_start], conversation_history
            )
            pair = messages[observation_start:]
            if len(pair) != 2:
                raise RuntimeError(
                    "required delegation observation must contain exactly one assistant call and one tool result"
                )
            inserted = session_db.append_messages_atomic(self.session_id, pair)
            if inserted != 2:
                raise RuntimeError("required delegation observation atomic append was incomplete")
            for message in pair:
    ...
    def _spawn_background_review(self, messages_snapshot: List[Dict], review_memory: bool = False,
        ...
        """
        # Switchboard-hosted ACP sessions must not fork a background review: the
        # fork would replay the full conversation on the session provider quota
        # and race the gateway's turn lifecycle.
        try:
            from gateway.session_context import get_session_env
    ...
    def _cancel_required_delegations_async(self) -> None:
        """Publish required-child cancellation off-thread so ACP cancel stays O(1)."""
        try:
    ...
    def interrupt(
        ...
        *,
        ...
        """Forward upstream interrupt handling and cancel owned ACP children."""
        admitted = InterruptControlMixin.interrupt(
            self,
            message,
        ...
        if admitted:
            self._cancel_required_delegations_async()
        return admitted

    def hard_interrupt(self, message: Optional[str] = None, *, tool_reason: Optional[str] = None) -> None:
        """Hard-stop wrapper; the upstream mixin intentionally bypasses dynamic dispatch."""
        InterruptControlMixin.interrupt(
            self, message, hard_cancel=True, tool_reason=tool_reason
        )
        self._cancel_required_delegations_async()

    def _reset_stream_delivery_tracking(self, *, discard_provisional_attempt: bool = False) -> None:
        """Flush scrubber tails through the ACP provisional-stream gate."""
        ...
        context_scrubber = getattr(self, "_stream_context_scrubber", None)
        if context_scrubber is not None:
            self._deliver_scrubbed_stream_delta(context_scrubber.flush())
        if discard_provisional_attempt and getattr(self, "_acp_provisional_stream_active", False):
    ...
    def _record_streamed_assistant_text(self, text: str) -> None:
        ...
        if self._stream_writer_superseded() or self._has_unconsumed_required_delegations():
            return
    ...
    def _deliver_scrubbed_stream_delta(self, text: str) -> None:
        """Deliver scrubbed text, or hold it until the ACP response is classified."""
        if not text or self._has_unconsumed_required_delegations():
            return
        if self._stream_writer_superseded():
            self._note_dropped_stream_writer("_deliver_scrubbed_stream_delta")
            return
        if getattr(self, "_acp_provisional_stream_active", False):
            buffer = list(getattr(self, "_acp_provisional_stream_buffer", []))
            buffer.append(("stream_delta", text))
            self._acp_provisional_stream_buffer = buffer
            return
        callbacks = [
            cb
            for cb in (
        ...
        StreamDeliveryMixin._enqueue_stream_hook(
            self, "on_stream_delta", delta=text, kind="text"
        )
        if delivered:
            self._record_streamed_assistant_text(text)

    def _fire_stream_delta(self, text: str) -> None:
        """Scrub and deliver text while honoring ACP required/provisional gates."""
        if isinstance(text, str) and text:
            self._touch_activity("receiving assistant response text", meaningful=True)
        ...
        if isinstance(text, str):
            ...
            if (
                ...
                and not getattr(self, "_streamed_assistant_text_parts", None)
            ):
                text = text.lstrip("\n")
        if not text:
            return
    ...
    def _fire_tool_gen_started(self, tool_name: str) -> None:
        ...
        if tool_name:
            self._touch_activity(f"receiving tool call: {tool_name}", meaningful=True)
    ...
    def _fire_streamed_codex_commentary(self, text: str) -> None:
        ...
        if getattr(self, "_acp_provisional_stream_active", False):
            buffer = list(getattr(self, "_acp_provisional_stream_buffer", []))
            buffer.append(("codex_commentary", text))
            self._acp_provisional_stream_buffer = buffer
            return
        callback = getattr(self, "interim_assistant_callback", None)
        if callback is None or not isinstance(text, str):
            return
    ...
    def _emit_interim_assistant_message(self, assistant_msg: Dict[str, Any]) -> None:
        ...
        if getattr(self, "_acp_provisional_stream_active", False):
            buffer = list(getattr(self, "_acp_provisional_stream_buffer", []))
            buffer.append(("interim_assistant", dict(assistant_msg)))
            self._acp_provisional_stream_buffer = buffer
            return
        ...
        for part in commentary_parts:
            key = StreamDeliveryMixin._normalize_interim_visible_text(part)
    ...
    def _finish_acp_provisional_stream(self, *, discard: bool) -> None:
        ...
        for entry in buffered:
            kind, payload = entry if isinstance(entry, tuple) and len(entry) == 2 else ("stream_delta", entry)
    ...
    def _has_unconsumed_required_delegations(self) -> bool:
        """Whether this ACP turn is still barred from emitting an answer."""
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        # Keep the live model path and registry fallback on one policy. ACP
        # roots supervise required children to completion in the same turn.
        return _delegate_task(
            ...
            required=_model_required_value(self), action=function_args.get("action"),
