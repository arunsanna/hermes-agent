...
# These controller tools are available only to the ACP parent turn that owns
# required delegation. Keep them out of broad aliases such as ``all`` while
# still making them explicit members of the ACP surface.
_ACP_ONLY_TOOLS = frozenset({"delegation_status", "delegation_wait", "delegation_cancel"})
_HERMES_ACP_TOOLS = [t for t in _CODING_TOOLS if t != "clarify"] + sorted(_ACP_ONLY_TOOLS)
...
TOOLSETS = {
    ...
    "hermes-acp-child": _ts(
        "Required-delegation child surface — ACP coding tools without parent-only controls",
        [t for t in _HERMES_ACP_TOOLS if t not in _ACP_ONLY_TOOLS],
    ),
