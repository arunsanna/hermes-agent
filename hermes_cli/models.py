...
# Synapse publishes the accepted reasoning levels per model and rejects unknown levels. Keep a
# short-lived process cache so request-time clamping does not turn into a catalog fetch per turn.
    ...
    with _synapse_catalog_lock:
        ...
        if cached is None or cached[0] <= now:
            catalogue = _fetch_synapse_capabilities(
                base_url=key, api_key=os.environ.get("SYNAPSE_API_KEY") or None, timeout=timeout)
            ttl = _SYNAPSE_CATALOG_TTL_S if catalogue is not None else _SYNAPSE_CATALOG_FAILURE_TTL_S
            cached = (now + ttl, catalogue)
            _synapse_catalog_cache[key] = cached
    catalogue = cached[1]
    return None if catalogue is None else catalogue.get(str(model or "").strip())
...
def synapse_wire_reasoning_effort(
    reasoning_config: Optional[dict], *, model: Optional[str], base_url: Optional[str]
) -> Optional[str]:
    """Return Synapse's top-level ``reasoning_effort``, clamped to its live catalog."""
