...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Soft turn time budget (minutes) before the model is nudged to pause,
        # summarize progress, and ask the user whether to continue. The nudge
        # fires once per turn at the next tool-round boundary; the user's reply
        # resumes as an ordinary next turn. 0 disables. An explicit
        # ``run_budget_seconds`` uses its own stronger wrap-up path instead.
