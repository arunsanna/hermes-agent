...
class SessionMessagesMixin:
    ...
    def append_messages_atomic(self, session_id: str, messages: List[Dict[str, Any]]) -> int:
        """Append an already-validated protocol group in one SQLite transaction.

        Required-delegation observations are an assistant tool call followed by its tool result;
        they must either both persist or neither persist.
        """
    ...
    def rewrite_message_content(
        self, session_id: str, tool_call_id: str, content: Any, *, expected_content: Any,
    ) -> bool:
        """Compare-and-swap one active tool result and clear its stale API sidecar."""
        if not session_id or not tool_call_id:
            return False
        encoded_content = self._encode_content(content)
        encoded_expected = self._encode_content(expected_content)
        def _do(conn):
            cur = conn.execute(
                "UPDATE messages SET content = ?, api_content = NULL "
                "WHERE session_id = ? AND tool_call_id = ? AND role = 'tool' "
                "AND active = 1 AND content = ?",
                (encoded_content, session_id, tool_call_id, encoded_expected),
            )
