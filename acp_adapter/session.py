...
class SessionManager:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # Load conversation history. Poisoned transcripts must reach the
        # self-healer byte-for-byte: generic alternation repair now prunes
        # unanswered assistant tool calls, which would erase the exact
        # protocol-only candidate the correction marker exists to recover.
        # Healthy restores still repair alternation before live replay.
        try:
            history = db.get_messages_as_conversation(
                session_id, repair_alternation=not poisoned
            )
        except Exception:
