...
def _async_background_delegation_id(
    ...
    """Return the id from an accepted asynchronous ``delegate_task`` dispatch."""
...
def _is_async_background_dispatch(result: Optional[str], tool_name: str | None = None) -> bool:
    """Whether a delegate result is only an accepted background dispatch ack."""
...
def build_async_background_completion(
    ...
    """Close a background dispatch card when its aggregate result arrives."""
...
def flush_async_background_dispatches(
    ...
    """Terminalize dispatch cards that remain open when the turn ends."""
    updates: List[ToolCallProgress] = []
...
def _todo_items(data: Args) -> list | None:
    return next(
        (
            data[key]
            for key in ("todos", "todo_list", "todo")
...
def _title_todo(args: Args) -> str:
    items = _todo_items(args)
    return f"todo ({_plural(len(items), 'item')})" if items is not None else "todo"
...
def _start_todo(args: Args) -> str:
    items = next(
        (args[key] for key in ("todos", "todo_list", "todo") if isinstance(args.get(key), list)),
        None,
    )
    if items is None:
...
def _delegate_raw_input(arguments: Args) -> Dict[str, Any]:
    """Build bounded machine-readable input for a delegate dispatch."""
    ...
    if isinstance(tasks, list) and tasks:
        ...
        if bounded:
            raw_args["tasks"] = bounded
    else:
        goal = str(arguments.get("goal") or "").strip()
        role = str(arguments.get("role") or "").strip()
        if goal:
            raw_args["goal"] = _truncate_text(goal, limit=400)
        if role:
            raw_args["role"] = role
    if arguments.get("background"):
        raw_args["background"] = True
...
def _build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    ...
    raw_input = _delegate_raw_input(arguments) if tool_name == "delegate_task" else None
...
def build_tool_complete(
    ...
    if tool_name == "delegate_task":
        data = _json_loads_maybe(result)
        if isinstance(data, dict) and isinstance(data.get("delegationAttempt"), dict):
            delegation_attempt = data
