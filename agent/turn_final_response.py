...
def finish_text_response(
    ...
    required_pending = agent._has_unconsumed_required_delegations()
    agent._finish_acp_provisional_stream(discard=required_pending)
    if required_pending:
        # A child completion can race this provider response. The candidate
        # was generated without the child evidence, so never persist or expose
        # it as final: durably observe the child and synthesize once more.
