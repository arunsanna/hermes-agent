...
class CodexAppServerSession:
    ...
    def _drive_turn(
        ...
        ``pre_scope_filter`` then turn scope, handed to ``on_note``. ``before_poll``
        runs only when both inbound queues are empty, so queued activity wins a
        watchdog boundary race. Hooks return True to complete the turn. Deadline
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            if note is not None and self._is_stale_notification(note):
                logger.info(
                    ...
                    note.get("method", ""), self._notification_turn_id(note), result.turn_id,
                )
                continue
            if note is None:
