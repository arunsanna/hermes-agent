...
def _ensure_codex_session(agent) -> None:
    ...
    agent._codex_session = CodexAppServerSession(
        ...
        reasoning_effort=(
            str(getattr(agent, "reasoning_config", {}).get("effort") or "").strip() or None
            if isinstance(getattr(agent, "reasoning_config", None), dict)
            and getattr(agent, "reasoning_config", {}).get("enabled") is not False
            else None
        ),
...
class _CodexResponseAssembler:
    ...
    def __init__(self, *, model, on_text_delta, on_reasoning_delta, on_commentary_message,
                 on_tool_start=None, on_tool_delta=None, on_first_delta=None):
        ...
        self.on_tool_start, self.on_tool_delta = on_tool_start, on_tool_delta
    ...
    def _on_item_added(self, event: Any, event_type: str) -> None:
        ...
        if "function_call" in str(item_type):
            ...
            tool_name = _event_field(item, "name", "")
            if tool_name:
                self._safe(self.on_tool_start, "on_tool_start", str(tool_name))
    ...
    def _on_function_call(self, event: Any, event_type: str) -> None:
        ...
        if "delta" in event_type:
            delta_args = _event_field(event, "delta", "") or ""
            if delta_args:
                self._safe(self.on_tool_delta, "on_tool_delta", str(delta_args))
...
def _consume_codex_event_stream(
    ...
    on_tool_start=None, on_tool_delta=None, on_first_delta=None, on_event=None, interrupt_check=None,
) -> SimpleNamespace:
    ...
    assembler = _CodexResponseAssembler(
        ...
        on_commentary_message=on_commentary_message, on_tool_start=on_tool_start,
        on_tool_delta=on_tool_delta, on_first_delta=on_first_delta,
    )
...
def run_codex_stream(agent, api_kwargs: dict, client: Any = None, on_first_delta=None):
    ...
    def _on_tool_delta(_delta: str) -> None:
        agent._touch_activity("receiving tool call arguments", meaningful=True)
    ...
    def _on_event(event: Any) -> None:  # TTFB/activity touch — once per SSE event.
        ...
        # Generic envelopes prove transport liveness; content callbacks mark
        # meaningful activity for required-delegation supervision.
        agent._touch_activity("receiving stream response", meaningful=False)
        ...
        try:
            try:
                ...
                final = _consume_codex_event_stream(
                    ...
                    on_reasoning_delta=_fenced(_on_reasoning_delta), on_commentary_message=on_commentary_message,
                    on_tool_start=_fenced(_on_tool_start), on_tool_delta=_fenced(_on_tool_delta),
