...
def _commit_tool_result(
    ...
    *,
    ...
    if not blocked:
        ...
        _safe_callback(
            agent.tool_progress_callback, "Tool progress",
            "tool.completed", function_name, None, None, tool_call_id=tool_call_id,
...
def _publish_sequential_result(
    agent, messages: list, ref: _ToolCallRef, managed: _ManagedToolResult,
    *, tool_duration: float, index: int, budget: BudgetConfig,
) -> tuple[bool, Optional[str]]:
    """Terminal hook → observe → commit → completion callbacks/print for one sequential
    result. Returns ``(published, direct_output)``; ``published`` is False when
    the incremental flush failed and the caller must stop the batch."""
    ...
    direct_output = (
        None
        if managed.blocked
        else extract_terminal_direct_output(ref.name, ref.args, display_function_result)
    )
...
def execute_tool_calls_sequential(
    ...
    for i, tool_call in enumerate(tool_calls, 1):
        ...
        published, direct_output = _publish_sequential_result(
            agent, messages, ref, managed,
            tool_duration=tool_duration, index=i, budget=_tool_budget,
        )
        if not published:
            return
        if finalize and len(tool_calls) == 1:
    ...
    if finalize:
        ...
        if tail_after_steer != tail_before_steer:
            direct_response = None
