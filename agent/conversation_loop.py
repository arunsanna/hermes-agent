...
from agent.turn_context import PreflightCompressionTimedOut, build_turn_context, drop_stale_api_content
...
def _observe_required_delegations(
    ...
    *,
    ...
    while True:
        ...
        # One turn can own several required batches. Re-check until every
        # terminal record has been durably observed and consumed.
        continue
...
def _maybe_request_turn_checkin(agent, messages) -> None:
    ...
    if getattr(agent, "_turn_checkin_fired", False):
        return
    # An explicit run budget provides the stronger upstream wrap-up/deadline
    # behavior. Do not inject a second, competing notice into the same turn.
...
def run_conversation(
    ...
    # A prior abnormal ACP turn may have escaped before its required result
    # became durable. Cancel and consume that exact owner while its old turn id
    # is still bound; build_turn_context assigns the new owner below.
    if str(getattr(agent, "platform", "") or "").lower() == "acp":
        try:
            from tools.async_delegation import stop_required_for_agent
            ...
            return {
                ...
                "api_calls": 0,
                "error": "required_delegation_observation_failed",
                "turn_exit_reason": "required_delegation_observation_failed",
            }
        try:
            agent._finish_acp_provisional_stream(discard=True)
        except Exception:
            logger.debug(
                "Previous provisional stream cleanup failed", exc_info=True
            )
    ...
    # Fatal provider/guardrail/budget exits can bypass the normal no-tool
    # answer gate. Never leave a live required owner behind: make its evidence
    # durable before finalization or fail closed with no assistant answer.
    try:
        if agent._has_unconsumed_required_delegations():
            agent._finish_acp_provisional_stream(discard=True)
            prior_exit_reason = str(s._turn_exit_reason or "unknown")
            if not _observe_required_delegations(
                agent,
                s.messages,
                ...
                s._turn_exit_reason = "required_delegation_interrupted"
            else:
                s.final_response = None
                if prior_exit_reason in {"unknown", "budget_exhausted"}:
                    s._turn_exit_reason = "budget_exhausted"
                else:
                    agent._required_observation_failed = True
                    s.failed = True
                    s._turn_exit_reason = (
        ...
        s.failed = True
        s.final_response = None
        s._turn_exit_reason = "required_delegation_observation_failed"
