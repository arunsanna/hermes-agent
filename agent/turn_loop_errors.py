...
def handle_outer_loop_error(
    ...
    # A local processing exception can occur after delegate_task registered a
    # required controller record. Do not retry or return through that gap:
    # close provisional output, join the child, and persist its observation.
    try:
        from agent.conversation_loop import (
