...
def run_tool_round(
    ...
    if _tvv.action == "return":
        from agent.conversation_loop import (
            _clear_orphaned_required_launch,
            _required_safe_terminal_result,
        )
        ...
        return _verdict(
            "return",
            _required_safe_terminal_result(
                agent, _tvv.result, messages, conversation_history
            ),
        )
    if _tvv.action == "continue":
        from agent.conversation_loop import _clear_orphaned_required_launch
        ...
        try:
            ...
            _direct_output_requested = (
                ...
                and terminal_direct_output_requested(
                    _direct_call.function.name, _direct_args
                )
            )
        except Exception:
            logger.debug(
                "Direct terminal-output classification failed", exc_info=True
            )
    if _required_launch:
        # The dispatch handler replaces this response-local latch with a
        # controller-owned record. Until then every visible sink stays closed.
        agent._required_delegation_launching = True
    required_response_guard = (
        _required_launch or agent._has_unconsumed_required_delegations()
    )
    agent._finish_acp_provisional_stream(
        discard=(required_response_guard or _direct_output_requested)
    )
    ...
    assistant_msg, duplicate_previous_interim = stage_tool_call_message(
        ...
        messages=messages,
        required_response_guard=required_response_guard,
    )
    ...
    from agent.conversation_loop import (
        _observe_required_delegations,
        _required_safe_terminal_result,
    )

    # Consume any required result that finished during this batch without
    # blocking; pending work remains model-supervisable on the next iteration.
    if not _observe_required_delegations(
        agent, messages, conversation_history, wait_for_pending=False
    ):
        interrupted = True
    ...
    if agent._tool_guardrail_halt_decision is not None:
        ...
        if _required_launch:
            # A guardrail halt after required dispatch must never emit ordinary
            # controlled-halt prose as though child evidence were synthesized.
            if not _observe_required_delegations(
                agent, messages, conversation_history, wait_for_pending=True
            ):
                return _verdict(
                    "return",
                    _required_safe_terminal_result(
                        agent,
                        {
                            ...
                            "completed": False,
                            "interrupted": True,
                            "error": "required_delegation_interrupted",
                        },
                        messages,
...
def stage_tool_call_message(
    agent: Any, *, assistant_message: Any, finish_reason: Any, messages: Any,
    required_response_guard: bool = False,
    ...
    turn_content = "" if required_response_guard else (assistant_message.content or "")
