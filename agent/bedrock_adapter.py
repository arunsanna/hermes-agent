...
def stream_converse_with_callbacks(
    ...
    on_interrupt_check=None, on_event=None, on_tool_delta=None,
) -> SimpleNamespace:
    ...
    paths); ``on_tool_delta`` receives nonempty toolUse input JSON fragments;
    ``on_interrupt_check`` True stops streaming; ``on_event`` fires for EVERY event before branching
    ...
    for event in event_stream.get("stream", []):
        ...
        if "contentBlockStart" in event:
            ...
            if "text" in delta:
                ...
                current_tool["input_json"] += input_delta
                if input_delta and on_tool_delta:
                    on_tool_delta(input_delta)
