...
def _hook_timeout_s() -> float:
    """Per-callback timeout for response-gating lifecycle hooks."""
    try:
...
def _invoke_hook_safely(name: str, logger: logging.Logger, **kwargs) -> list:
    ...
    try:
        ...
        return invoke_hook(name, _timeout_s=_hook_timeout_s(), **kwargs)
    ...
    if not required_gate_blocked:
        try:
            ...
            if str(getattr(agent, "platform", "") or "").lower() == "acp":
                ...
                logger.exception("Required delegation final gate check failed")
    if required_gate_blocked:
        # Do not expose a normal completion while a required child result was
        # not durably observed; the next ACP turn may take ownership only after
        # this structured failure has been surfaced.
        final_response = None
        failed = True
