...
def interruptible_api_call(agent, api_kwargs: dict):
    """Non-streaming provider call with an explicit required-delegation in-flight marker."""
...
class _StreamingCall:
    ...
    def _route_suppressed_text(self, text: str) -> None:
        ...
        if getattr(self.agent, "_acp_provisional_stream_active", False):
            self.agent._fire_stream_delta(text)
    ...
    def _count_chunk(self, diag, chunk) -> None:
        ...
        self.last_chunk_time["t"] = time.time()
        self.agent._touch_activity("receiving stream response", meaningful=False)
        with contextlib.suppress(Exception):
    ...
    def _open_chat_stream(self, stream_kwargs: dict[str, Any]):
        ...
        self.agent._touch_activity(
            "waiting for provider response (streaming)", meaningful=False
        )
        ...
        for chunk in _iter_provider_stream_chunks(stream, response=lambda: self._attempt_stream_response):
            ...
            if delta_tool_calls:
                ...
                for tc_delta in delta_tool_calls:
                    tc_function = getattr(tc_delta, "function", None)
                    if tc_function is not None and getattr(tc_function, "arguments", None):
                        self.agent._touch_activity(
                            "receiving tool call arguments", meaningful=True
                        )
        ...
        try:
            for event in stream:
                ...
                if event_type == "content_block_start":
                    ...
                    elif delta_type == "input_json_delta" and getattr(delta, "partial_json", ""):
                        self.agent._touch_activity(
                            "receiving tool call arguments", meaningful=True
                        )
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    """Streaming provider call with an explicit required-delegation in-flight marker."""
    agent._touch_activity(
