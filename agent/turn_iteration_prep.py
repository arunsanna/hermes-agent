...
def begin_iteration(
    ...
    # Consume only already-terminal results between model iterations. Pending
    # children keep the answer gate closed while status/wait/cancel remain
    # available to the parent model.
    ...
    agent._touch_activity(
        f"starting API call #{api_call_count}", meaningful=False
    )
    if (
        ...
        # The model may emit visible text before its delegate_task delta. Hold
        # the entire root ACP response until its final shape is known so a
        # required delegation cannot leak a stale pre-child candidate.
