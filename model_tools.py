...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        # The platform-neutral ``all`` alias deliberately excludes controls
        # that require an ACP root turn. Iterating every toolset would add
        # those controls back through ``hermes-acp``.
        tools.update(resolve_toolset("all"))
...
def _execute_tool(function_name: str, function_args: Dict[str, Any], original_args: Dict[str, Any], ids: _CallIds,
                  *, user_task: Optional[str], enabled_tools: Optional[List[str]], parent_agent: Optional[Any],
...
def handle_function_call(
    ...
    # Managed ACP sessions attach the live parent so session-owned protocol
    # controls can verify their runtime contract. Ordinary callers remain
    # fail-open if the optional ACP integration is unavailable.
    if parent_agent is not None:
        try:
            ...
            logger.debug(
                "Switchboard orchestration runtime gate unavailable for %s; continuing",
                function_name,
                exc_info=True,
            )
            runtime_block = None
    ...
    try:
        ...
        result = _execute_tool(function_name, function_args, original_args, ids, user_task=user_task,
                               enabled_tools=enabled_tools, parent_agent=parent_agent,
