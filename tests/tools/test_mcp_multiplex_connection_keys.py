...
def two_profiles(tmp_path, monkeypatch):
    ...
    ledgers = ("_servers", "_server_scope_keys", "_server_tool_scopes", "_server_connecting",
               ...
               "_server_trust_levels", "_server_trust_levels_scoped", "_tool_read_only_hints",
               "_mcp_tool_server_names",
               "_mcp_tool_server_scoped", "_parallel_safe_servers_scoped",
               "_switchboard_parent_scoped_read_only_tools", "_switchboard_parent_scoped_raw_tools",
               "_switchboard_parent_scoped_full_servers", "_switchboard_parent_scoped_full_tools",
               "_orphaned_adopters")
...
def test_same_named_server_with_other_credentials_is_a_separate_connection(two_profiles):
    ...
    cfg_a.update({
        "switchboard_parent_read_only_tools": ["t"],
    ...
    assert reg.get_switchboard_parent_tool_names() == {"mcp__x__t"}
    ...
    assert reg.get_switchboard_parent_tool_names() == set()
...
def test_owner_reload_reregisters_profiles_that_adopted_its_connection(two_profiles):
    ...
    cfg = {
        ...
        "switchboard_parent_tools": "all",
        "supports_parallel_tool_calls": True,
    }
    scope_a = two_profiles("a")
    srv_a = _server("x", cfg)
    srv_a._tools[0].annotations = {"readOnlyHint": False}
    disc._select_new_servers({"x": cfg})
    disc._adopt_server("x", srv_a)
    srv_a._registered_tool_names = reg._register_server_tools("x", srv_a, cfg)
    disc._note_connect_success("x")
    tool_name = "mcp__x__t"
    assert reg.get_switchboard_parent_tool_names() == {tool_name}
    assert disc.is_mcp_tool_parallel_safe(tool_name) is True
    ...
    restricted_cfg = {
        **cfg,
        "trust": "untrusted",
        "switchboard_parent_tools": None,
    }
    assert reg.register_connected_into_current_scope({"x": restricted_cfg}) == 1
    assert reg.get_switchboard_parent_tool_names() == set()
    assert disc.is_mcp_tool_parallel_safe(tool_name) is True
    with patch("tools.approval_prompt.request_elicitation_consent", return_value="deny"):
        assert "did not approve" in handlers._trust_gate_check("x", "t")

    two_profiles("a")
    assert reg.get_switchboard_parent_tool_names() == {tool_name}
    assert handlers._trust_gate_check("x", "t") is None
    ...
    for registered_name in list(srv_a._registered_tool_names):
        reg._deregister_mcp_tool_all_scopes(srv_a, registered_name)
    assert reg.get_switchboard_parent_tool_names() == set()
    assert disc.is_mcp_tool_parallel_safe("mcp__x__t") is False
    with core._lock:
