...
def test_switchboard_parent_selection_requires_explicit_config_and_read_only_hint():
    import tools.mcp_tool_registration as mcp_registration
    ...
    try:
        mcp_registration._record_tool_trust_metadata("webull", config, tools)
        mcp_registration._track_mcp_tool_server(
        ...
        assert mcp_registration.get_switchboard_parent_read_only_tool_names() == {
            read_registry
        }
    finally:
        for name in (read_registry, write_registry, unselected_registry):
            mcp_registration._forget_mcp_tool_server(name)
...
def test_switchboard_parent_all_selection_includes_write_capable_tools():
    ...
    try:
        mcp_registration._record_tool_trust_metadata(
            "webull", {"switchboard_parent_tools": "all"}, tools
        )
        mcp_registration._track_mcp_tool_server(read_registry, "webull", "get_watchlists")
        mcp_registration._track_mcp_tool_server(write_registry, "webull", "place_order")

        assert mcp_registration.get_switchboard_parent_read_only_tool_names() == set()
        assert mcp_registration.get_switchboard_parent_tool_names() == {
            read_registry,
            write_registry,
        }
    finally:
        for name in (read_registry, write_registry):
            mcp_registration._forget_mcp_tool_server(name)
        mcp_registration._record_tool_trust_metadata("webull", {}, [])
